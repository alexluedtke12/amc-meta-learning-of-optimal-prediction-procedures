    ...
    for scenario in ['interior','boundary']:
        for n in [100,500]:
            ...
            for i in range(num_mc):
                ...
                for j in range(len(n_metatrain_list)):
                    n_metatrain = n_metatrain_list[j]
                    T_out = T_list[j](w_tilde,w,y).squeeze().detach()
...
df.to_csv('tables/linear_results_all.csv', index=False)
df.round(2).to_csv('tables/linear_results_all_rounded.csv', index=False)
